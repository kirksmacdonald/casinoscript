      ...
      */
	  ...
	  if ($module_freekassa_query['status'] == 1) {
		  ...
		  if ($_POST['mode'] == "pay_freekassa") {	  
              ...
              $login = $_SESSION['login'];
              $summa = intval($_POST['summa']);
              @mysql_query("INSERT INTO pay_deposits VALUES('', '{$login}', '{$summa}', '{$date}','{$time}', '{$ip}','FREEKASSA','','0','Пополнение счёта freekassa для Пользователя:{$login} , Сумма:{$summa} Кредитов','')");
              $pay_id    = mysql_insert_id();
