...
define ( "MY_TABLE_BANK",   "game_bank"  ) ;//00 таблица банков 
define ( "MY_BANK_NAME",    "name"       ) ;//01 название банка
define ( "MY_BANK_SUMM",    "bank"       ) ;//02 сумма банка 
define ( "MY_BANK_PROC",    "proc"       ) ;//03 процент (для совместимости со старыми версиями)
define ( "MY_BANK_MAX",     "winmax"     ) ;//04 макмимальная сумма Win за 1 спин 
define ( "MY_BANK_IN",      "income"     ) ;//05 ПРОЦЕНТ КОТОРЫЙ ЗАБИРАЕМ СЁБЕ 
define ( "MY_BANK_BONUS",   "bonus"      ) ;//06 сумма бонуса 
define ( "MY_BANK_BONUS2",  "bonus2"     ) ;//07 сумма второго бонуса 
define ( "MY_BANK_BONREADY","bonusready" ) ;//08 сумма готовности выдачи бонуса
define ( "MY_BANK_BONPROC", "bonusproc"  ) ;//09 процент отчисляемый в банк бонуса 
define ( "MY_BANK_BONMODE", "bonusmode"  ) ;//10 процент отчисляемый в банк бонуса 
define ( "MY_BANK_JP",      "jackpot"    ) ;//11 сумма банка  JP
define ( "MY_BANK_JPPROC",  "jpotproc"   ) ;//12 процент отчисляемый в банк  JP 
define ( "MY_BANK_MODE",    "mode"       ) ;//13 настройки слота 
define ( "MY_BANK_MODE2",   "mode2"      ) ;//14 остояние слота  
define ( "MY_BANK_MODE3",   "mode3"      ) ;//15 функции слота 
define ( "MY_BANK_MODE4",   "mode4"      ) ;//16 расширеные функции слота 
define ( "MY_BANK_INCASH",  "incash"     ) ;//17 общий дохот слота                              
