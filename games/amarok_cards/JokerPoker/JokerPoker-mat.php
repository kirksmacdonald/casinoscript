...
//Комбинации карт
$config['combos']=array();
$config['combos']['royal_flush']=array('payout_type' => 9, 'payout' => 500);
$config['combos']['five_of_kind']=array('payout_type' => 8, 'payout' => 100);
$config['combos']['joker_royal_flush']=array('payout_type' => 7, 'payout' => 50);
$config['combos']['straight_flush']=array('payout_type' => 6, 'payout' => 50);
$config['combos']['four_of_kind']=array('payout_type' => 5, 'payout' => 20);
$config['combos']['full_house']=array('payout_type' => 4, 'payout' => 8);
$config['combos']['flush']=array('payout_type' => 3, 'payout' => 5);
$config['combos']['straight']=array('payout_type' => 2, 'payout' => 4);
$config['combos']['three_of_kind']=array('payout_type' => 1, 'payout' => 2);
$config['combos']['two_pairs']=array('payout_type' => 0, 'payout' => 1);
...
function new_card ($t=true) {
     global $_SESSION;
  mt_srand();
  $id=mt_rand(0, count($_SESSION['JokerPoker_cardsmap'])-1);
  $result=$_SESSION['JokerPoker_cardsmap'][$id];
  if ($t) {
   unset($_SESSION['JokerPoker_cardsmap'][$id]);
   $cm=array();
   $i=0;
   foreach ($_SESSION['JokerPoker_cardsmap'] as $v) {
    $cm[$i]=$v;
    $i++;
   }
   $_SESSION['JokerPoker_cardsmap']=$cm;
  }
  return $result;
}
...
function check_cards ($cards) {
  ...
  if (is_royal_flash($cards)) {
    return "royal_flush";
  } elseif (is_five_of_a_kind($cards)) {
    return "five_of_kind";
  } elseif (is_joker_royal_flash($cards)) {
    return "joker_royal_flush";
...
switch ($_POST['ACTION']) {
  ...
  $_SESSION['JokerPoker_game_lasttime']=time();
  unset($_SESSION['JokerPoker_cards']);
  break;

  case "EXIT":
  unset($_SESSION['JokerPoker_game_lasttime']);
  unset($_SESSION['JokerPoker_cards']);
  unset($_SESSION['JokerPoker_cardsmap']);
  break;
  ...
  $_SESSION['JokerPoker_cardsmap']=make_map();
  $_SESSION['JokerPoker_cards']=array(new_card(), new_card(), new_card(), new_card(), new_card());
  $_SESSION['JokerPoker_game_lasttime']=time(); //И обновим счётчик времени.

  print_var("RESULT", "OK");
  print_var("BALANCE", user_balance());
  print_var("CARDS", implode("|", $_SESSION['JokerPoker_cards']));
  ...
  $combo=false;
  $winall=0;
  $map=$_SESSION['JokerPoker_cardsmap'];
  while (!$possible_result) {
   $_SESSION['JokerPoker_cardsmap']=$map;
   $winall=0;
   if ($holdcards[0]==0) {
     print_var("NEWCARD0", "1");
     $_SESSION['JokerPoker_cards'][0]=new_card();
   }
   if ($holdcards[1]==0) {
     print_var("NEWCARD1", "1");
     $_SESSION['JokerPoker_cards'][1]=new_card();
   }
   if ($holdcards[2]==0) {
     print_var("NEWCARD2", "1");
     $_SESSION['JokerPoker_cards'][2]=new_card();
   }
   if ($holdcards[3]==0) {
     print_var("NEWCARD3", "1");
     $_SESSION['JokerPoker_cards'][3]=new_card();
   }
   if ($holdcards[4]==0) {
     print_var("NEWCARD4", "1");
     $_SESSION['JokerPoker_cards'][4]=new_card();
   }
