...
define("_GAME_NAME", "CrazyDoctors");

if (eregi("doctors_spin\.asp$", strtolower(__FILE__))===false) {
...
if (!isset($_SESSION['CrazyDoctors_wheels'])) {  print_var("t_alert1", "Internal Error, my darling.");
  print_var("error", "1");
  die();
}
...
while (true) {
  ...
  //if ($full_bank/(_AMAROK_DELITEL/(_AVERAGE_BET*$game_configs['coinsize']))>(_AVERAGE_BET*$game_configs['coinsize'])*_AVERAGE_WIN) {
	  ...
	  if (($must_bonus) AND (isset($scatter_dices[$scatters]))) {	    $possible_bonus_win=_AVERAGE_BONUS_DICE_WIN*$scatter_dices[$scatters];
...
//  }
  ...
  if (($game_ok) AND ($bonus_ok)) {  	break;  # как бэ если результат нас не устроил то кружимся дальше - варриантов тонна)
  }
}
...
function wheel_positions () {
  $result=array(
  	  mt_rand(1, $_SESSION['CrazyDoctors_wheel_1_size']),  # Первый барабан
  	  mt_rand(1, $_SESSION['CrazyDoctors_wheel_2_size']),
  	  mt_rand(1, $_SESSION['CrazyDoctors_wheel_3_size']),
  	  mt_rand(1, $_SESSION['CrazyDoctors_wheel_4_size']),
  	  mt_rand(1, $_SESSION['CrazyDoctors_wheel_5_size'])   # Пятый барабан
  );
...
function render_map ($positions) {
  ...
  foreach ($positions as $wheel => $position) {
    $real_wheel=$wheel+1;
    if ($position==1) {
      $result[]=$_SESSION['CrazyDoctors_wheels'][$real_wheel][$_SESSION["CrazyDoctors_wheel_".$real_wheel."_size"]];
      $result[]=$_SESSION['CrazyDoctors_wheels'][$real_wheel][$position];
      $result[]=$_SESSION['CrazyDoctors_wheels'][$real_wheel][$position+1];
    } elseif ($position==$_SESSION["CrazyDoctors_wheel_".$real_wheel."_size"]) {
      $result[]=$_SESSION['CrazyDoctors_wheels'][$real_wheel][$position-1];
      $result[]=$_SESSION['CrazyDoctors_wheels'][$real_wheel][$position];
      $result[]=$_SESSION['CrazyDoctors_wheels'][$real_wheel][1];
    } else {
      $result[]=$_SESSION['CrazyDoctors_wheels'][$real_wheel][$position-1];
      $result[]=$_SESSION['CrazyDoctors_wheels'][$real_wheel][$position];
      $result[]=$_SESSION['CrazyDoctors_wheels'][$real_wheel][$position+1];
