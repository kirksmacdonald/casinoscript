...
define("_GAME_NAME", "JamesBand");

if (eregi("james_spin\.asp$", strtolower(__FILE__))===false) {
...
if (!isset($_SESSION['JamesBand_wheels'])) {  print_var("t_alert2", "Internal Error, my darling.");
  print_var("error", "2");
  die();
}
...
if ((isset($_SESSION['JamesBand_freegames'])) AND (($_SESSION['JamesBand_freegames']) AND ($_SESSION['JamesBand_freegames_times']>0))) {
  # И да. Я грешен, мне лениво править и накручивать лишние переменные, так что я эмулирую.
  $_POST['nb_lines']=$_SESSION['JamesBand_freegames_nb_lines'];
  $_POST['nb_lines']=$_SESSION['JamesBand_freegames_nb_lines'];
  $bet_coins=$_POST['nb_coins']*$_POST['nb_lines'];
  $bet=$bet_coins*$game_configs['coinsize'];
} else {  if ((!isset($_POST['nb_lines'])) OR (($_POST['nb_lines']<1) OR ($_POST['nb_lines']>9))) {
    print_var("error", "1");
    die();
  }
...
while (true) {
  ...
  if ((isset($_SESSION['JamesBand_freegames'])) AND (($_SESSION['JamesBand_freegames']==true) AND ($_SESSION['JamesBand_freegames_times']>0))) {
	...
	$must_win=chance(Game::$config['win']);
    $must_free_games=chance($game_configs['free_game_chance']);    //$bank=bank_balance($game_configs['game_bank'])/$bank_delitel;
	$bank=Game::bankBalance()/$bank_delitel;
  ...
  //if ($full_bank/(_AMAROK_DELITEL/(_AVERAGE_BET*$game_configs['coinsize']))>(_AVERAGE_BET*$game_configs['coinsize'])*_AVERAGE_WIN) {
      if ((!$must_win) AND (($winall==0) OR (($bet>$winall) AND ($winall<=$bank)))) {  	    $game_ok=true;
      }

	  if (($must_win) AND (($winall<=$bank) AND ($winall>$bet))) {	    $game_ok=true;
	  }

	  if (($must_free_games) AND (isset($scatter_free_games[$scatters]))) {	    $free_ok=true;
	  }
...
if ((isset($_SESSION['JamesBand_freegames'])) AND (($_SESSION['JamesBand_freegames']) AND ($_SESSION['JamesBand_freegames_times']>0))) {
  ...
  if ($win_coins>0) {    bank_pay($game_configs['free_game_bank'], $winall);
    user_add($winall);
  }
...
if ((isset($scatter_free_games[$scatters])) AND ((!isset($_SESSION['JamesBand_freegames'])) OR ($_SESSION['JamesBand_freegames']==false))) {  trace_str("");
  # Типа мы тока начали
  # Ща я буду зарабатывать все деньги мира, для любимой тигры!
  # Боже, как же меня прет
  $_SESSION['JamesBand_freegames']=true;
  $_SESSION['JamesBand_freegames_win_total']=0;
  $_SESSION['JamesBand_freegames_nb_coins']=$_POST['nb_coins'];
  $_SESSION['JamesBand_freegames_nb_lines']=$_POST['nb_lines'];
  $_SESSION['JamesBand_freegames_times']=$scatter_free_games[$scatters];
  $_SESSION['JamesBand_freegames_times_played']=0;

  print_var("total_win_free", $_SESSION['JamesBand_freegames_win_total']);

  print_var("mode_free_begin", "1");
  print_var("mode_free", "1");

  print_var("nb_gratuit_to_play", $_SESSION['JamesBand_freegames_times']);
  print_var("nb_gratuit_played", $_SESSION['JamesBand_freegames_times_played']);
} elseif ($_SESSION['JamesBand_freegames']) {
  # Продолжаем веселье
  if (isset($scatter_free_games[$scatters])) {
    $_SESSION['JamesBand_freegames_times']+=$scatter_free_games[$scatters];
  }

  if ($_SESSION['JamesBand_freegames_times']>0) {
    $_SESSION['JamesBand_freegames_win_total']+=$winall;    $_SESSION['JamesBand_freegames_times']--;
    $_SESSION['JamesBand_freegames_times_played']++;
    ...
    if ($_SESSION['JamesBand_freegames_times']>0) {
      ...
      $_SESSION['JamesBand_freegames']=false;
    }

    print_var("nb_gratuit_to_play", $_SESSION['JamesBand_freegames_times']);
    print_var("nb_gratuit_played", $_SESSION['JamesBand_freegames_times_played']);
  } else {
    $_SESSION['JamesBand_freegames']=false;    print_var("total_win_free", $_SESSION['JamesBand_freegames_win_total']);
	print_var("mode_free_begin", "0");
    print_var("mode_free", "0");
    print_var("nb_gratuit_to_play", $_SESSION['JamesBand_freegames_times']);
    print_var("nb_gratuit_played", $_SESSION['JamesBand_freegames_times_played']);
    $_SESSION['JamesBand_freegames_win_total']=0;
    $_SESSION['JamesBand_freegames_times']=0;
    $_SESSION['JamesBand_freegames_times_played']=0;
    unset($_SESSION['JamesBand_freegames_nb_coins'], $_SESSION['JamesBand_freegames_nb_lines']);
  }
} else {  print_var("total_win_free", "0");
  print_var("mode_free_begin", "0");
  print_var("mode_free", "0");
...
function render_map ($positions) {
  ...
  foreach ($positions as $wheel => $position) {
    if ($position==1) {
      $result[]=$_SESSION['JamesBand_wheels'][$wheel+1][_WHEEL_SIZE];
      $result[]=$_SESSION['JamesBand_wheels'][$wheel+1][$position];
      $result[]=$_SESSION['JamesBand_wheels'][$wheel+1][$position+1];
    } elseif ($position==_WHEEL_SIZE) {
      $result[]=$_SESSION['JamesBand_wheels'][$wheel+1][$position-1];
      $result[]=$_SESSION['JamesBand_wheels'][$wheel+1][$position];
      $result[]=$_SESSION['JamesBand_wheels'][$wheel+1][1];
    } else {
      $result[]=$_SESSION['JamesBand_wheels'][$wheel+1][$position-1];
      $result[]=$_SESSION['JamesBand_wheels'][$wheel+1][$position];
      $result[]=$_SESSION['JamesBand_wheels'][$wheel+1][$position+1];
