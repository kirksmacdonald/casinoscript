...
if (eregi("james_init\.asp$", strtolower(__FILE__))===false) {
...
$_SESSION['JamesBand_wheels']=array();

for ($wheel=1; $wheel<6; $wheel++) {
  $_SESSION['JamesBand_wheels'][$wheel]=array();
  for ($icon=1; $icon<_WHEEL_SIZE+1; $icon++) {
   $_SESSION['JamesBand_wheels'][$wheel][$icon]=mt_rand(_MIN_ICON, _MAX_ICON);
   if ($icon!=1) {
     while ($_SESSION['JamesBand_wheels'][$wheel][$icon]==$_SESSION['JamesBand_wheels'][$wheel][$icon-1]) {
        $_SESSION['JamesBand_wheels'][$wheel][$icon]=mt_rand(_MIN_ICON, _MAX_ICON);
     }
   }
  }
