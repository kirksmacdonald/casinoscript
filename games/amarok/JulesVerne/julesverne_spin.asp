...
define("_GAME_NAME", "JulesVerne");

if (eregi("julesverne_spin\.asp$", strtolower(__FILE__))===false) {
...
if (!isset($_SESSION['JulesVerne_wheels'])) {
...
if ((isset($_SESSION['JulesVerne_freegames'])) AND (($_SESSION['JulesVerne_freegames']) AND ($_SESSION['JulesVerne_freegames_times']>0))) {
  // И да. Я грешен, мне лениво править и накручивать лишние переменные, так что я эмулирую.
  $_POST['nb_lines']=$_SESSION['JulesVerne_freegames_nb_lines'];
  $_POST['nb_lines']=$_SESSION['JulesVerne_freegames_nb_lines'];
...
while (true) {
  ...
  if ((isset($_SESSION['JulesVerne_freegames'])) AND (($_SESSION['JulesVerne_freegames']==true) AND ($_SESSION['JulesVerne_freegames_times']>0))) {
...
if ((isset($scatter_free_games[$scatters])) AND ((!isset($_SESSION['JulesVerne_freegames'])) OR ($_SESSION['JulesVerne_freegames']==false))) {
  trace_str("");
  // Типа мы тока начали
  // Ща я буду зарабатывать все деньги мира, для любимой тигры!
  // Боже, как же меня прет
  $_SESSION['JulesVerne_freegames']=true;
  $_SESSION['JulesVerne_freegames_win_total']=0;
  $_SESSION['JulesVerne_freegames_nb_coins']=$_POST['nb_coins'];
  $_SESSION['JulesVerne_freegames_nb_lines']=$_POST['nb_lines'];
  $_SESSION['JulesVerne_freegames_times']=$scatter_free_games[$scatters];
  $_SESSION['JulesVerne_freegames_times_played']=0;

  print_var("total_win_free", $_SESSION['JulesVerne_freegames_win_total']);

  print_var("mode_free_begin", "1");
  print_var("mode_free", "1");

  print_var("nb_gratuit_to_play", $_SESSION['JulesVerne_freegames_times']);
  print_var("nb_gratuit_played", $_SESSION['JulesVerne_freegames_times_played']);
} elseif ($_SESSION['JulesVerne_freegames']) {
  // Продолжаем веселье
  if (isset($scatter_free_games[$scatters])) {
    $_SESSION['JulesVerne_freegames_times']+=$scatter_free_games[$scatters];
  }

  if ($_SESSION['JulesVerne_freegames_times']>0) {
    $_SESSION['JulesVerne_freegames_win_total']+=$winall;
    $_SESSION['JulesVerne_freegames_times']--;
    $_SESSION['JulesVerne_freegames_times_played']++;
    ...
    if ($_SESSION['JulesVerne_freegames_times']>0) {
      ...
      $_SESSION['JulesVerne_freegames']=false;
    }
    ...
    print_var("nb_gratuit_played", $_SESSION['JulesVerne_freegames_times_played']);
    $_SESSION['JulesVerne_freegames_win_total']=0;
    $_SESSION['JulesVerne_freegames_times']=0;
    $_SESSION['JulesVerne_freegames_times_played']=0;
    unset($_SESSION['JulesVerne_freegames_nb_coins'], $_SESSION['JulesVerne_freegames_nb_lines']);
  }
} else {
...
function render_map ($positions) {
  ...
  foreach ($positions as $wheel => $position) {
    if ($position==1) {
      $result[]=$_SESSION['JulesVerne_wheels'][$wheel+1][_WHEEL_SIZE];
      $result[]=$_SESSION['JulesVerne_wheels'][$wheel+1][$position];
      $result[]=$_SESSION['JulesVerne_wheels'][$wheel+1][$position+1];
    } elseif ($position==_WHEEL_SIZE) {
      $result[]=$_SESSION['JulesVerne_wheels'][$wheel+1][$position-1];
      $result[]=$_SESSION['JulesVerne_wheels'][$wheel+1][$position];
      $result[]=$_SESSION['JulesVerne_wheels'][$wheel+1][1];
    } else {
      $result[]=$_SESSION['JulesVerne_wheels'][$wheel+1][$position-1];
      $result[]=$_SESSION['JulesVerne_wheels'][$wheel+1][$position];
      $result[]=$_SESSION['JulesVerne_wheels'][$wheel+1][$position+1];
