...
if (eregi("vegas_spin\.asp$", strtolower(__FILE__))===false) {
...
if (!isset($_SESSION['LasVegas_wheels'])) {
...
function wheel_positions () {
  $result=array(
  	  mt_rand(1, $_SESSION['LasVegas_wheel_1_size']),  // Первый барабан
  	  mt_rand(1, $_SESSION['LasVegas_wheel_2_size']),
  	  mt_rand(1, $_SESSION['LasVegas_wheel_3_size']),
  	  mt_rand(1, $_SESSION['LasVegas_wheel_4_size']),
  	  mt_rand(1, $_SESSION['LasVegas_wheel_5_size'])   // Пятый барабан
  );
...
function render_map ($positions) {
  ...
  foreach ($positions as $wheel => $position) {
    $real_wheel=$wheel+1;
    if ($position==1) {
      $result[]=$_SESSION['LasVegas_wheels'][$real_wheel][$_SESSION["LasVegas_wheel_".$real_wheel."_size"]];
      $result[]=$_SESSION['LasVegas_wheels'][$real_wheel][$position];
      $result[]=$_SESSION['LasVegas_wheels'][$real_wheel][$position+1];
    } elseif ($position==$_SESSION["LasVegas_wheel_".$real_wheel."_size"]) {
      $result[]=$_SESSION['LasVegas_wheels'][$real_wheel][$position-1];
      $result[]=$_SESSION['LasVegas_wheels'][$real_wheel][$position];
      $result[]=$_SESSION['LasVegas_wheels'][$real_wheel][1];
    } else {
      $result[]=$_SESSION['LasVegas_wheels'][$real_wheel][$position-1];
      $result[]=$_SESSION['LasVegas_wheels'][$real_wheel][$position];
      $result[]=$_SESSION['LasVegas_wheels'][$real_wheel][$position+1];
