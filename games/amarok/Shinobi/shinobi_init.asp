...
define("_GAME_NAME", "Shinobi");

if (eregi("shinobi_init\.asp$", strtolower(__FILE__))===false) {
...
$_SESSION['Shinobi_wheels']=array();


for ($wheel=1; $wheel<6; $wheel++) {
  $_SESSION['Shinobi_wheels'][$wheel]=array();
  $_SESSION["Shinobi_wheel_".$wheel."_size"]=mt_rand($game_configs['wheel_size_min'], $game_configs['wheel_size_max']);
  print_var("wheel".$wheel."size", $_SESSION["Shinobi_wheel_".$wheel."_size"]);
  print_var("wheel".$wheel."Pos", mt_rand(1, $_SESSION["Shinobi_wheel_".$wheel."_size"]));
  for ($icon=1; $icon<$_SESSION["Shinobi_wheel_".$wheel."_size"]+1; $icon++) {
   $_SESSION['Shinobi_wheels'][$wheel][$icon]=mt_rand(_MIN_ICON, _MAX_ICON);
   if ($icon!=1) {
     while ($_SESSION['Shinobi_wheels'][$wheel][$icon]==$_SESSION['Shinobi_wheels'][$wheel][$icon-1]) {
        $_SESSION['Shinobi_wheels'][$wheel][$icon]=mt_rand(_MIN_ICON, _MAX_ICON);
     }
   }
   print_var("wheel".$wheel."_".$icon, $_SESSION['Shinobi_wheels'][$wheel][$icon]);
  }
}
