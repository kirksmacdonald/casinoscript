...
if (eregi("ny_spin\.asp$", strtolower(__FILE__))===false) {
...
if (!isset($_SESSION['New-York_wheels'])) {
...
function wheel_positions () {
  $result=array(
  	  mt_rand(1, $_SESSION['New-York_wheel_1_size']),  // Первый барабан
  	  mt_rand(1, $_SESSION['New-York_wheel_2_size']),
  	  mt_rand(1, $_SESSION['New-York_wheel_3_size']),
  	  mt_rand(1, $_SESSION['New-York_wheel_4_size']),
  	  mt_rand(1, $_SESSION['New-York_wheel_5_size'])   // Пятый барабан
  );
...
function render_map ($positions) {
  ...
  foreach ($positions as $wheel => $position) {
    $real_wheel=$wheel+1;
    if ($position==1) {
      $result[]=$_SESSION['New-York_wheels'][$real_wheel][$_SESSION["New-York_wheel_".$real_wheel."_size"]];
      $result[]=$_SESSION['New-York_wheels'][$real_wheel][$position];
      $result[]=$_SESSION['New-York_wheels'][$real_wheel][$position+1];
    } elseif ($position==$_SESSION["New-York_wheel_".$real_wheel."_size"]) {
      $result[]=$_SESSION['New-York_wheels'][$real_wheel][$position-1];
      $result[]=$_SESSION['New-York_wheels'][$real_wheel][$position];
      $result[]=$_SESSION['New-York_wheels'][$real_wheel][1];
    } else {
      $result[]=$_SESSION['New-York_wheels'][$real_wheel][$position-1];
      $result[]=$_SESSION['New-York_wheels'][$real_wheel][$position];
      $result[]=$_SESSION['New-York_wheels'][$real_wheel][$position+1];
