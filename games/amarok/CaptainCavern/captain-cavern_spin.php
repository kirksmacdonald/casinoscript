...
if (!isset($_SESSION['CaptainCavern_wheels'])) {
...
$bet=$bet_coins;
if (Game::userBalance()<$bet) {
  printVar("error", "1");
  die();
}


$bank_delitel=Game::$config['delitel']/$bet;
...
while (true) {
  ...
  for ($i=1; $i<$_POST['nb_lines']+1; $i++) {
	  ...
	  if ($wilds>0) {
	    ...
	    if (isset($on_line_wins_config[$char][$c])) {
	      $char=$on_line_wins_config[$char][$c]*2;  # потому что присутствие вилда удваевает выигрыш с линии
	    ...
	    if (isset($on_line_wins_config[$char][$c])) {
	      if ($doubled) {
	        $lines_wincoins[$i]=$_POST['nb_coins']*$on_line_wins_config[$char][$c]*2;
	      } else {
  ...
  $must_bonus=chance(Game::$config['bonus']);
  ...
  $winall=$win_coins;
	  ...
	  if (($must_bonus) AND (isset($scatter_dices[$scatters]))) {
	    ...
	    $bonus_bank==Game::bankBalance()/$bank_delitel;
...
Game::writeStat($bet, $winall);


printVar("error", "0");
printVar("betamount", $bet);
printVar("money_win_total", $winall);
printVar("nb_coins_win_total", $win_coins);
printVar("credit", Game::userBalance());
printVar("coinsize", 1);
printVar("nb_coins", $_POST['nb_coins']);
printVar("nb_lines", $_POST['nb_lines']);
printVar("scatter_win", $scatter_wincoins);

for ($i=1;$i<10;$i++) {
  if (isset($lines_wincoins[$i])) {
    printVar("line_".$i."_wincoins", $lines_wincoins[$i]);
  } else {
    printVar("line_".$i."_wincoins", "0");
  }
}


printVar("wheel1Pos", $wheels[0]);
printVar("wheel2Pos", $wheels[1]);
printVar("wheel3Pos", $wheels[2]);
printVar("wheel4Pos", $wheels[3]);
printVar("wheel5Pos", $wheels[4]);

$wilds_on_screen=false;
for ($i=1; $i<16; $i++) {
  if ((isset($active_wilds[$i])) AND ($active_wilds[$i])) {
    printVar("active_wild".$i, "1");
    $wilds_on_screen=true;
  } else {
    printVar("active_wild".$i, "0");
    $wilds_on_screen=false;
  }
}
for ($i=1;$i>10;$i++) {
  if (isset($lines_symbwin[$i])) {
    printVar("line_9_nbsymbwin", $lines_symbwin[$i]);
  } else {
    printVar("line_9_nbsymbwin", "ppppp");
  }
}
...
if (isset($scatter_dices[$scatters])) {
  ...
  printVar("nb_dice_to_play", $scatter_dices[$scatters]);
...
function wheel_positions() {
  $result=array(
  	  mt_rand(1, $_SESSION['CaptainCavern_wheel_1_size']),  # Первый барабан
  	  mt_rand(1, $_SESSION['CaptainCavern_wheel_2_size']),
  	  mt_rand(1, $_SESSION['CaptainCavern_wheel_3_size']),
  	  mt_rand(1, $_SESSION['CaptainCavern_wheel_4_size']),
  	  mt_rand(1, $_SESSION['CaptainCavern_wheel_5_size'])   # Пятый барабан
  );
  return $result;
}
...
function render_map($positions) {
  ...
  foreach ($positions as $wheel => $position) {
    $real_wheel=$wheel+1;
    if ($position==1) {
      $result[]=$_SESSION['CaptainCavern_wheels'][$real_wheel][$_SESSION["CaptainCavern_wheel_".$real_wheel."_size"]];
      $result[]=$_SESSION['CaptainCavern_wheels'][$real_wheel][$position];
      $result[]=$_SESSION['CaptainCavern_wheels'][$real_wheel][$position+1];
    } elseif ($position==$_SESSION["CaptainCavern_wheel_".$real_wheel."_size"]) {
      $result[]=$_SESSION['CaptainCavern_wheels'][$real_wheel][$position-1];
      $result[]=$_SESSION['CaptainCavern_wheels'][$real_wheel][$position];
      $result[]=$_SESSION['CaptainCavern_wheels'][$real_wheel][1];
    } else {
      $result[]=$_SESSION['CaptainCavern_wheels'][$real_wheel][$position-1];
      $result[]=$_SESSION['CaptainCavern_wheels'][$real_wheel][$position];
      $result[]=$_SESSION['CaptainCavern_wheels'][$real_wheel][$position+1];
