...
if (eregi("paris_spin\.asp$", strtolower(__FILE__))===false) {
...
if (!isset($_SESSION['Paris_wheels'])) {
...
function wheel_positions () {
  $result=array(
  	  mt_rand(1, $_SESSION['Paris_wheel_1_size']),  // Первый барабан
  	  mt_rand(1, $_SESSION['Paris_wheel_2_size']),
  	  mt_rand(1, $_SESSION['Paris_wheel_3_size']),
  	  mt_rand(1, $_SESSION['Paris_wheel_4_size']),
  	  mt_rand(1, $_SESSION['Paris_wheel_5_size'])   // Пятый барабан
  );
...
function render_map ($positions) {
  ...
  foreach ($positions as $wheel => $position) {
    $real_wheel=$wheel+1;
    if ($position==1) {
      $result[]=$_SESSION['Paris_wheels'][$real_wheel][$_SESSION["Paris_wheel_".$real_wheel."_size"]];
      $result[]=$_SESSION['Paris_wheels'][$real_wheel][$position];
      $result[]=$_SESSION['Paris_wheels'][$real_wheel][$position+1];
    } elseif ($position==$_SESSION["Paris_wheel_".$real_wheel."_size"]) {
      $result[]=$_SESSION['Paris_wheels'][$real_wheel][$position-1];
      $result[]=$_SESSION['Paris_wheels'][$real_wheel][$position];
      $result[]=$_SESSION['Paris_wheels'][$real_wheel][1];
    } else {
      $result[]=$_SESSION['Paris_wheels'][$real_wheel][$position-1];
      $result[]=$_SESSION['Paris_wheels'][$real_wheel][$position];
      $result[]=$_SESSION['Paris_wheels'][$real_wheel][$position+1];
