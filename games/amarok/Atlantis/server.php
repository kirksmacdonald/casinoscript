...
/*
  ...
  */
  $l = 'admin1';
$dur = (aisset($_SESSION['dur'])) ? $_SESSION['dur'] : 'wmr';
$multiplier = 100;
$bon1multiplier = 100;
 ...
 if ( isset ( $_POST["cacheKiller"] ) )
  {
   ...
   $result = mysql_query("select * from clients where login='$l'");
   $row = mysql_fetch_array( $result );
   $result = mysql_query("select * from games_bank where name='$gamegb'");
   ...
   // $wildon=1;
   // mode
  $trim_mode=mysql_fetch_array(mysql_query( "SELECT *, substring(cast(mode as char),10,1) as mode1, substring(cast(mode as char),9,1) as mode2, substring(cast(mode as char),8,1) as mode3, substring(cast(mode as char),7,1) as mode4 FROM `games_bank` WHERE name = '$gamegb' "));
   //mysql_query($sqls);
   ...
   if ( ($bonustype[0] == "btype") && ( $statgame == "Bonus1" or $statgame == "Bonus2") )
     ...
     if (  $cxcase >= 1  && $bonustype[1] == "1" )
      {
        ...
        // 1 - casino
		//if ($dur == "wmr")
         mysql_query("update clients set mode='$bonusnum|$bonuspos[1]:$bonustyp[1]:$bonussum[1]|$bonuspos[2]:$bonustyp[2]:$bonussum[2]|$bonuspos[3]:$bonustyp[3]:$bonussum[3]|$bonuspos[4]:$bonustyp[4]:$bonussum[4]|$bonuspos[5]:$bonustyp[5]:$bonussum[5]|$bonuspos[6]:$bonustyp[6]:$bonussum[6]|' where login='$l'");
     ...
     if ( $bon2win > 0 )//****************************************************************************************************************************
      {
               ...
               //   echo "&BCP==$bonuscup";
		if ($dur == "wmr") 
			mysql_query("update clients set cash=cash+'$bon2win' where login='$l'");//**********************************************************************
		else 
		...
		if ($dur == "wmr") {
			if ( $mode0001 == "on" )// mode public cash
			 {
			   mysql_query("update games_bank set bank=bank-'$bon2win'
	where name='ttuz'");//**************************************************************
			 }
	...
	where name='25linb2'");//**********************************
			...
			if ( $mode0004 == "on" )//mode private slot cash
			 {
			   mysql_query("update games_bank set bonus=bonus-'$bon2win'
	where name='$gamegb'");//**********************************
			 }
       // end cash mode 
	   }
       ...
       // mysql_query("update clients set cash=cash+'$bon2win', mode='1|xx:xx:xxx|xx:xx:xxx|xx:xx:xxx|xx:xx:xxx|xx:xx:xxx|' where login='$l'");//*********
       //                                           номер бонус игры|  |  |   |
       //                                                     позиция  |  |   |
      ...
      $rowd = mysql_fetch_array(mysql_query("select * from clients where login='$l'"));
  ...
  if ( $statgame == "spin" )
   ...
   // при проверке надо учесть чтобы $allbet был равен $incoins * $inlines !!! важно
    ...
    if ($mode0010 == "on")
     ...
     //mysql_query("update clients set cash=cash-'$allbet', win_double='not', holdset='$allbet| $linebet|$linecon|' where login='$l'");
	 if ($dur == "wmr") {
       mysql_query("update clients set cash=cash-'$allbet', win_double='not', holdset='$allbet|$coinson |$linecon|' where login='$l'");

  		// cash mode
    if ( $mode0001 == "on" )// mode public cash
     {         // 3 - casino
         mysql_query("update games_bank set bank=bank+'$bankin ',jackpot=jackpot+'$betjp' where name='ttuz'");
         mysql_query("update clients set bonus=bonus+'$betbn',incash=incash+'$betin' where name='$gamegb'");
     }
         if ($mode0002 == "on")//mode public 9 lines cash
     {
         mysql_query("update games_bank set jackpot=jackpot+'$betjp' where name='ttuz'");
         mysql_query("update games_bank set bank=bank+'$bankin',incash=incash+'$betin' where name='25linb2'");
         mysql_query("update clients set bonus=bonus+'$betbn' where name='$gamegb'");
     }
    if ($mode0004 == "on")//mode private slot cash
     {
         mysql_query("update games_bank set jackpot=jackpot+'$betjp' where name='ttuz'");
         mysql_query("update games_bank set bank=bank+'$bankin', bonus=bonus+'$betbn',incash=incash+'$betin' where name='$gamegb'");
     }// end cash mode
    ...
    if ( $allwin > 0 && $dur == 'wmr')
    {
        ...
        mysql_query("update clients set cash=cash+'$allwin' where login='$l'");

        // cash mode
        if ($mode0001 == "on")// mode public cash
         {
           mysql_query("update games_bank set bank=bank-'$allwin' where name='ttuz'");
         }
        if ($mode0002 == "on")//mode public 25 lines cash
         {
           mysql_query("update games_bank set bank=bank-'$allwin' where name='25linb2'");
         }
        if ($mode0004 == "on")//mode private slot cash
         {
           mysql_query("update games_bank set bank=bank-'$allwin' where name='$gamegb'");
         }
        // end cash mode
    } else { //virtual cash (fun)
      ...
      //-----------------
              // 4 - casino
	//if ($dur == "wmr")
      mysql_query("update clients set mode2='btype|$btype|bscatter|$bscatter', mode='$bonusnum|$bonuspos[1]:$bonustyp[1]:$bonussum[1]|$bonuspos[2]:$bonustyp[2]:$bonussum[2]|$bonuspos[3]:$bonustyp[3]:$bonussum[3]|$bonuspos[4]:$bonustyp[4]:$bonussum[4]|$bonuspos[5]:$bonustyp[5]:$bonussum[5]|$bonuspos[6]:$bonustyp[6]:$bonussum[6]|' where login='$l'");
